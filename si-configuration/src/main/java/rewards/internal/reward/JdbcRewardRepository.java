 ...
 */
public class JdbcRewardRepository extends SimpleJdbcDaoSupport implements
		RewardRepository {
	...
	public RewardConfirmation confirmReward(AccountContribution contribution,
		...
		getSimpleJdbcTemplate().update(sql, confirmationNumber,
	...
	public RewardConfirmation findConfirmationFor(Dining dining) {
		...
		List<RewardConfirmation> confirmationsOrNull = getSimpleJdbcTemplate()
				.query(sql1, new RewardConfirmationMapper(), dining.getTransactionId());
	...
	private class RewardConfirmationMapper implements RowMapper<RewardConfirmation> {
		@Override
		public RewardConfirmation mapRow(ResultSet rs, int i)
				throws SQLException {
			return new RewardConfirmation(rs.getString(1), rs.getString(2),
					new AccountContribution(rs.getString(3), MonetaryAmount
							.valueOf(rs.getString(4)), null));
		}
	}
